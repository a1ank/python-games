...
def collide(zerg1, zerg2): #<--- чтоб два объекта не заезжали друг на друга
...
img1 = pygame.image.load('soldier1.png') #<---------upload png
img2 = pygame.image.load('soldier2.png') #<---------upload png
img3 = pygame.image.load('soldier3.png') #<---------upload png
img4 = pygame.image.load('soldier4.png') #<---------upload png
img5 = pygame.image.load('soldier5.png') #<---------upload png

img1 = pygame.transform.scale(img1, (50, 75))
img2 = pygame.transform.scale(img2, (55, 60))
img3 = pygame.transform.scale(img3, (60, 60))
img4 = pygame.transform.scale(img4, (75, 50))
img5 = pygame.transform.scale(img5, (60, 60))
...
while not done:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                 ...
                 zerg.zx = zerg.x
                 zerg.zy = zerg.y
                 zerg.img = images[random.randrange(0, len(images), 1)]
                 zerg.w, zerg.h = zerg.img.get_rect().size
                 for z in zergs:
                     if collide(z,zerg):
                          zerg.x = random.randrange(100, 400, 1)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                ...
                if selected is not None:
                    ...
                    selected.zx = selected.zx + zerg.w/20
                    selected.zy = selected.zy + zerg.h/20
            else:
                if selected is not None:
    ...
    for zerg in zergs: # для каждого зерга
        for other_zerg in zergs:
            if other_zerg != zerg:
                if zerg.bullet is not None:
                    x,y = zerg.bullet
    ...
    for z in zergs:
        ...
        if z.life > 50:
            ...
            pygame.draw.rect(screen, (244,25,10), [z.x, z.y - 10, z.w/100 * z.life, 8], 0)
    for z in zergs:
        screen.blit(z.img, (z.x, z.y))
